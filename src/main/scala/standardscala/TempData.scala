...
case class TempData(day: Int, doy: Int, month: Int, year: Int, precip: Double, snow: Double, tave: Double,
...
object TempData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (rainySum_, rainyCount2_) = data.par.aggregate(0.0 -> 0)(
    ...
    val monthlyTemp = monthlyGroups.map { case (n, days) =>
      n -> days.foldLeft(0.0)((sum, td) => sum + td.tmax) / days.length
    }
