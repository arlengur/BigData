...
import swiftvis2.plotting.Plot.scatterPlotWithLines
...
object RDDTempData extends JFXApp {
  ...
  val (rainySum_, rainyCount2_) = data.aggregate(0.0 -> 0)(
    { case ((sum, count), td) => if (td.precip < 1.0) (sum, count) else (sum + td.tmax, count + 1) }, { case ((s1, c1), (s2, c2)) => (s1 + s2, c1 + c2) })

  println(s"Average rainy temp is ${rainySum_ / rainyCount2_}")
  ...
  val monthlyGroups = data.groupBy(_.month)
  val monthlyHighTemp = monthlyGroups.map { case (n, days) =>
    n -> days.foldLeft(0.0)((sum, td) => sum + td.tmax) / days.size
  }
  val monthlyLowTemp = monthlyGroups.map { case (n, days) =>
    n -> days.foldLeft(0.0)((sum, td) => sum + td.tmin) / days.size
  }
  ...
  val longTermPlot = scatterPlotWithLines(averageByYearData.map(_._1),
    averageByYearData.map { case (_, (s, c)) => (s/c) }, symbolSize = 0.0, symbolColor = BlackARGB,
    lineGrouping = 1.0)
  FXRenderer(longTermPlot, 800, 600)

  val plot = Plot.scatterPlots(Seq(
    (monthlyHighTemp.map(_._1).collect(), monthlyHighTemp.map(_._2).collect(), RedARGB, 5.0),
    (monthlyLowTemp.map(_._1).collect(), monthlyLowTemp.map(_._2).collect(), BlueARGB, 5.0)
